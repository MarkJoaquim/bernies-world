...
signal lobby_sealed()
signal lobby_list_update(lobbies: Array)
...
func _parse_msg() -> bool:
	...
	if typeof(parsed) != TYPE_DICTIONARY or not parsed.has("type") or not parsed.has("id") or \
		(typeof(parsed.get("data")) != TYPE_STRING and typeof(parsed.get("data")) != TYPE_ARRAY):
		return false
	...
	if type == Message.ID:
		...
		lobby_list_update.emit(msg.data)
	else:
		return false

	return true  # Parsed.

func list_lobbies() -> Error:
	return _send_msg(Message.LIST, 0)

func create_lobby(_lobby: String) -> Error:
	return _send_msg(Message.CREATE, 0 if mesh else 1, _lobby)
